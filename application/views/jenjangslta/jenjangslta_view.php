<!-- Datatable style -->
  ...
  <div class="box">
   <div class="box-header">
     <h3 class="box-title">Master Data Jenjang SLTA</h3>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('jenjangslta/ajax_list')?>",
            "type": "POST"
        },
...
function edit_record(id)
{
    ...
    $.ajax({
        url : "<?php echo base_url('jenjangslta/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
 
            $('[name="idjenjangslta"]').val(data.idjenjangslta);
            $('[name="kodejenjangslta"]').val(data.kodejenjangslta);
            $('[name="deskripsi"]').val(data.deskripsi);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Ubah Jenjang SLTA'); // Set title to Bootstrap modal title
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('jenjangslta/ajax_add')?>";
    } else {
        url = "<?php echo site_url('jenjangslta/ajax_update')?>";
    }
    ...
    $.ajax({
        ...
        success: function(data)
        {
            if (data.hasil !== "sukses") {
                $("#error_kodejenjangslta").html(data.error.kodejenjangslta);
                $("#error_deskripsi").html(data.error.deskripsi);
            }
...
function delete_record(id)
{
    if(confirm('Apakah anda yakin akan menghapus data?'))
    {
        ...
        $.ajax({
            url : "<?php echo base_url('jenjangslta/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Form Jenjang SLTA</h3>
            </div>
            <div class="modal-body form">
                <form action="#" id="form" class="form-horizontal">
                    <input type="hidden" value="" name="idjenjangslta"/> 
                    <div class="form-body">
                        <div class="form-group">
                            <label class="control-label col-md-4">Kode Jenjang SLTA</label>
                            <div class="col-md-8">
                                <input name="kodejenjangslta" placeholder="Kode Jenjang SLTA" class="form-control" type="text">
                                <span class="text-danger" id="error_kodejenjangslta"></span>
                            </div>
                        </div>
                        <div class="form-group">
